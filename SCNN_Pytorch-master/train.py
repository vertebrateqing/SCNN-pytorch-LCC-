...
gpu_ids = [0,1,2,3,4,5,6,7]#一块gpu就改为[0]
...
def train(epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        img = sample['img'].cuda()#to(device)
        segLabel = sample['segLabel'].cuda()#to(device)
        exist = sample['exist'].cuda()#to(device)
        ...
        f=open("/public/home/lcc-dx03/SCNN_Pytorch-master/train.txt","a")#记录loss
        f.write("Train Epoch: {} ".format(epoch))
        f.write("batch loss: {:.3f}\n".format(loss.item()))
...
def val(epoch):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            ...
            f=open("/public/home/lcc-dx03/SCNN_Pytorch-master/val.txt","a")
            f.write("Val Epoch: {} ".format(epoch))
            f.write("batch loss: {:.3f} \n".format(loss.item()))
            #progressbar.update(1)
